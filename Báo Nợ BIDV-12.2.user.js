...
(function() {
    ...
    // Ngày
        const mNgay = vanBan.match(
            /(\d{2}\/\d{2}\/\d{4})\s+\d{2}:\d{2}:\d{2}\s+\d{1,3}(?:,\d{3})*\s+VND/i
        );
        ...
        // From Account: Số tài khoản của công ty
        const mFrom = vanBan.match(/Số tài khoản\s*(\d{6,20})/i);
        if (mFrom) {
            ketQua.fromAccount = mFrom[1];
        }

        // To Account: Tài khoản đối ứng trong diễn giải
        const mTo = vanBan.match(/TKTHE\s*:?\s*(\d{6,20})/i);
        if (mTo) {
            ketQua.toAccount = mTo[1];
        }
    // =========================
    // Số tiền
    // =========================
    const mTien = vanBan.match(/(\d{1,3}(?:,\d{3})*)\s*VND/i);
    if (mTien) {
        ketQua.soTien = mTien[1].replace(/,/g, "");
    }

    // =========================
    // Reference
    // Ví dụ:
    // -CTLNHIDO000015656203015-1/1-PMT-002
    // =========================
    ...
    function veBangDieuKhien() {
        ...
        container.style = `
            ...
            background: #260000; color: #a9b1d6; padding: 15px;
        ...
        container.innerHTML = `
            ...
            <button id="v9-btn-file" style="width: 100%; background: #bb9af7; color: #260000; border: none; padding: 10px; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: bold;">
                📁 CHỌN FILE BÁO NỢ BIDV
            </button>
