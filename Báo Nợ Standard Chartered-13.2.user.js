...
(function() {
    ...
    function timInputOdooTheoLabel(chuoiLabel) {
        ...
        const nameMap = {
            ...
            'to': ['to_account', 'x_to_account', 'partner_bank_id'],
    ...
    // 4. BỘ PHÂN TÍCH CHUỖI DÀNH RIÊNG CHO BÁO NỢ (DEBIT ADVICE)
    function trichXuatBaoNoStandardChartered(vanBan) {
        ...
        // [3] QUÉT TÀI KHOẢN NGUỒN - GIỮ LẠI CHỮ VND PHÍA TRƯỚC
        // Đưa chữ (VND) vào trong cặp ngoặc tròn bắt group thứ 1: (VND\d{8,15})
        ...
        if (mFromAcc) {
            ...
            // Dự phòng nếu không tìm thấy chữ VND liền kề, tự động ghép chữ VND vào trước số backup
            const mFromBackup = textChuan.match(/\b99451816999\b/);
            if (mFromBackup) {
                ketQua.fromAccount = "VND" + mFromBackup[0];
            }
        }

        // [4] QUÉT TÀI KHOẢN ĐÍCH (TO ACCOUNT) - TUYỆT CHIÊU QUÉT MẢNG LỌC TRỪ
        const tatCaDaySo = [...textChuan.matchAll(/\b\d{8,15}\b/g)].map(m => m[0]);

        let cácSoConLai = tatCaDaySo.filter(so => {
            // Lọc trừ: So sánh phần số thô, vì ketQua.fromAccount hiện tại đang chứa cả chữ "VND"
            if (ketQua.fromAccount && ketQua.fromAccount.includes(so)) return false;
            if (ketQua.soTien && so === ketQua.soTien) return false;
            if (so.startsWith('2026')) return false;
            return true;
        });

        if (cácSoConLai.length > 0) {
            ketQua.toAccount = cácSoConLai[0];
        }

        // [5] QUÉT DIỄN GIẢI (Payment Details)
        ...
        if (mDetail && mDetail[1].trim()) {
            ...
            if (mThanhToan) {
                ketQua.detail = mThanhToan[1].trim();
            }
        }

        // [6] QUÉT SỐ THAM CHIẾU (Bank Reference / Ref)
    ...
    function veBangDieuKhien() {
        ...
        container.innerHTML = `
            ...
            <div id="v9-log" style="font-size: 10px; color: #a9b1d6; margin-top: 10px; max-height: 120px; overflow-y: auto; background: #10101a; padding: 6px; border-radius: 4px; border: 1px solid #24283b; line-height: 1.4;">
                HỆ THỐNG v13.2: Đã fix lấy trọn vẹn chữ VND cho tài khoản nguồn.
            </div>
        `;
        ...
        document.getElementById('v9-file-input').addEventListener('change', async function(e) {
            ...
            try {
                ...
                const data = trichXuatBaoNoStandardChartered(vanBanRaw);
